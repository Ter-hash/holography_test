...
p.add_argument('--prop_model', type=str, default='ASM',
               help='Type of propagation model for reconstruction: ASM / MODEL / CAMERA/PARTIAL')
...
if opt.prop_model == "PARTIAL":
    prop_dists = (10 * cm, 10 * cm, 10 * cm)  # propagation distance from SLM plane to target plane
    wavelengths = (634.8 * nm, 510 * nm, 450 * nm)  # SLED
    if opt.src_type == 'LED':
        wavelengths = (633 * nm, 532 * nm, 460 * nm)[channel]  # LED
else:
    prop_dists = (20 * cm, 20 * cm, 20 * cm)  # propagation distance from SLM plane to target plane
...
if opt.prop_model == 'ASM':
    ...
    propagator = PhysicalProp(channel, laser_arduino=True, roi_res=(roi_res[1], roi_res[0]), slm_settle_time=0.15,
                              ...
                              # path of 21 x 12 calibration patterns, see Supplement.
                              show_preview=True)
    ...
    # print(opt.src_type)
    for c in chs:
        propagator = PartialProp(distance=prop_dists[c], feature_size=feature_size, batch_size=12,
                                 wavelength_central=wavelengths[c], num_wvls=15,
...
for k, target in enumerate(image_loader):
    ...
    imageio.imwrite(os.path.join(recon_path, f'{target_idx}_{run_id}_{chan_strs[channel]}.png'),
