...
class ModelPropagate(nn.Module):
    ...
    def __init__(self, distance=0.1, wavelength=520e-9, feature_size=6.4e-6, image_res=(1080, 1920), learn_dist=False,
        ...
        if content_field:
            self.content_dependent_field = ContentDependentField(num_layers=num_layers_cdp, num_features=num_feats_cdp,
                                                                 norm=nn.GroupNorm, latent_coords=latent_coords)
        else:
        ...
        if latent_coords:
            ...
            self.latent_coords = nn.Parameter(
                torch.from_numpy(np.stack((lx, ly), 0)).type(torch.float32).reshape(1, 2, *image_res),
                requires_grad=False)
        else:
