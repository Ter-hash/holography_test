...
p.add_argument('--run_id', type=str, default='1', help='Experiment name', required=True)
...
# p.add_argument('--num_iters', type=int, default=300, help='Number of iterations (GS, SGD)')
# parse arguments
opt = p.parse_args()
channel = opt.channel
run_id = opt.run_id
...
data_path = './datasets/train1080'  # path for training data
...
for i in range(opt.num_epochs):
    ...
    for k, target in enumerate(image_loader):
        ...
        print(f'[{i + 1}/{opt.num_epochs}], iteration [{ik}/{num_mse_iters}]: {loss_val.item()}')
